# Naton Design Philosophy

The aesthetic and technical rationale behind `Naton` — the "why", not the "how to
...
scanning, palette extraction — lives in the `natonctl` Go helper, not in QML. QML
invokes `natonctl` subcommands through `Process` objects and consumes JSON. The
